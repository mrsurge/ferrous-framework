...
`FerrousNativePeer` is the matching Rust peer-client. It connects to a Python or Ferrous controller using the same base URL plus `/fws_ws/socket.io`, authenticates as `role: "peer"`, tracks `fws_peer_subscriptions`, handles `fws_peer_request` for `fws.shell.input` by calling the local native manager write/EOF primitives, and returns the required Socket.IO ack DTO. It also starts peer relay workers that forward native lifecycle events and subscribed output chunks as `fws_peer_notification` messages. Log relay honors controller subscription hints and does not independently drain direct pipe/PTY stdout, so protocol readers keep ownership of stdout bytes.
